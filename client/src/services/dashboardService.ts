...
export interface RecentActivity {
  id: string;
  type: 'user_registered' | 'program_created' | 'event_created' | 'task_completed';
  description: string;
  timestamp: string;
  user?: string;
}
...
export const fetchDashboardStats = async (): Promise<DashboardStats> => {
  try {
    ...
    const tasksCompleted = allTasksSnapshot.size > 0 
 ...
 */
export const fetchRecentActivity = async (limitCount: number = 10): Promise<RecentActivity[]> => {
  try {
    ...
    recentProgramsSnapshot.forEach((doc) => {
      ...
      activities.push({
        ...
        description: `New program "${programData.title || programData.name}" created`,
    ...
    recentEventsSnapshot.forEach((doc) => {
      ...
      activities.push({
        id: `event_${doc.id}`,
        type: 'event_created',
        description: `New event "${eventData.title || eventData.name}" scheduled`,
